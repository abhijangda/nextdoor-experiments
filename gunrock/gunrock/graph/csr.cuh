        ...
        _FLAG | HAS_CSR, cudaHostRegisterFlag>
{
                                    ...
                                    bool quiet = false) 
    {
        ...
        this->edges = n_edges;
        this->directed = false;
        GUARD_CU(Allocate(vertex_ids.size (), n_edges, target));
        std::vector<SizeT> vertex_offsets;
        SizeT curr_offset = 0;
        GUARD_CU(Move (target, util::HOST));
        ...
        row_offsets[vertex_ids.size ()] = curr_offset;

        for (int v = 0; v < vertex_ids.size (); v++) {
            node_values[v] = vertex_ids[v];
        }


        GUARD_CU(Move (util::HOST, GRAPH_DEFAULT_TARGET));

        //std::cout << "Graph Loaded with ret val " << retval << std::endl;
        //Display ("", vertex_ids.size ());
        std::cout << "this->nodes " << this->nodes << " addr " << &(this->nodes) << " graph addr " << this << std::endl;
        return retval;
    }

    SizeT GetNodes () {return this->nodes;}
