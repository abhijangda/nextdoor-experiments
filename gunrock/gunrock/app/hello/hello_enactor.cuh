...
struct NeighborsIterationLoop
  ...
  cudaError_t Core(int peer_ = 0) {
    ...
    // </TODO>
    std::cout << "Running for hop " << hop << std::endl;
    if (hop == 0) {
...

      {
        ...
        auto advance_op = [
          ...
          neighbors[positions[src] + l] = dest;
          return false;
        };
      ...
      {        
        ...
        auto advance_op = [
                            prev_lengths, lengths
                              ...
                              const SizeT &edge_id, const VertexT &input_item,
                              const SizeT &input_pos, SizeT &output_pos) -> bool {
          atomicAdd (&lengths[src], prev_lengths[dest]);
          return false;
        };
      ...
      GUARD_CU(total_lengths.SetPointer(&h_total_lengths, 1*sizeof (SizeT), util::HOST));
      GUARD_CU(total_lengths.Move(util::DEVICE, util::HOST));
      std::cout << __LINE__ << ":  " << h_total_lengths << std::endl;
      data_slice.InitNeighborsForHop (h_total_lengths, hop, util::DEVICE);
      cpu_timer.Stop();
...

      {
        ...
        auto advance_op = [
                              ...
                              const SizeT &edge_id, const VertexT &input_item,
                              const SizeT &input_pos, SizeT &output_pos) -> bool {
          auto l = atomicAdd (&lengths[src], prev_lengths[dest]);
          for (int q = 0; q < prev_lengths[dest]; q++) {
            neighbors[positions[src] + l + q] = prev_neighbors[q + prev_positions[dest]];
          }
          return false;
        };
