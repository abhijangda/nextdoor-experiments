...
cudaError_t LoadGraph(util::Parameters &parameters, GraphT &graph,
  ...
  if (graph_type == "market")  // Matrix-market graph
  {
    ...
    GUARD_CU(graph.LoadFromFile(graph_file));
    graph.nodes = graph.GetNodes ();
    graph.edges = graph.csr().edges;
    graph.row_offsets = graph.csr().row_offsets;
    graph.column_indices = graph.csr().column_indices;
    graph.node_values = graph.csr().node_values;
    //graph.Display ("", graph.nodes);
    //graph.csr().Sort();
  }
  ...
  std::cout << __LINE__ << ": graph.nodes " << graph.nodes << " addr " << &graph.nodes << " graph-addr " << &graph << " getnodes " << graph.GetNodes () << " csr edges " << graph.edges << std::endl;
  if ((graph.FLAG & gunrock::graph::HAS_CSR) &&
      parameters.Get<bool>("sort-csr")) {
    graph.csr().Sort();
  }

  std::cout << __LINE__ << ": graph.nodes " << graph.nodes << std::endl;
  if (false && !parameters.Get<bool>("quiet")) {
